...
contract wallet {
    ...
    uint256 withdrawlAllowance;

    mapping(address => uint256) accounts; //Used in withdrawFunds function.
    mapping(address => uint256) allowances; //Used in setAnAllowance function.

    constructor() {
        walletOwner = msg.sender; //Set the wallet owner to who creates contract
    }

    function setAnAllowance(address payable _to, uint256 _allowance) public {
    ...
    function withdrawFunds(
        ...
        require(
            _from == msg.sender,
            "Withdrawls not allowed unless you are the wallet owner."
        ); //Check account owner.
        require(_amount <= accounts[msg.sender], "Insufficient funds!"); //Check for funds.

        if (allowances[msg.sender] > 0) {
            //Check withdrawl allowance.
            require(
                _amount <= allowances[msg.sender],
                "You are exceeding the allowance amount, aborting!"
            );
        }
